...
**Latest stable:** [`v3.1-stable`](https://github.com/CSUN-ARCS-GeoInfoVisualization/geo-info-data-visualization-project/releases/tag/v3.1-stable) — **physically-correct monotonic risk model** (HistGradientBoosting + monotonic constraints + a PDP physics gate, replacing the CalibratedRF that had learned backwards relationships — wind anti-correlated, humidity inverted, KBDI saturating) with a **continuous daily retraining pipeline** (GitHub-Actions FIRMS ingest → gated retrain → promote; dataset committed in-repo for zero DB cost-risk), **per-user saved zone overrides** on the researcher page (24h TTL, "Save for 24 hours" button, reset this/all zones, 20-zone shared total cap across all 4 zone types, EVI slider scale fix), a **live-data accuracy pass** (evac orders-vs-warnings split + 7-day active-only filter dropping stale/TEST zones + real `NOTES` cause; OPEN-only auto-refreshing shelters; REAL active-fires filter 77→7 dropping sub-acre fragments + out fires; enriched fire popups; active-fire totals on the dashboard + Shelters/Evac), and an **infrastructure overhaul** — a cache-durability fix that root-caused a 13-day stale-feed freeze (frozen `computed_at` + resilient saves + a force-recompute backstop), an auto-restart-after-backend-deploy workflow (Render deploys don't reliably restart gunicorn), and a build-time typecheck gate that blocks the undefined-reference crash class that white-screened the app. Builds on v3.0's four-channel alerts + email pipeline. See [`docs/SESSION_HANDOFF.md`](docs/SESSION_HANDOFF.md) to pick up where the team left off.

<details><summary>v3.0-stable (previous release)</summary>

**fourth alert channel: wildfires in your county** (per-county bundled CAL FIRE incident alerts every 10 min, change-driven dedup, one-final-"fully contained"-email-then-silence, "View updated perimeters on map" CTA), **universal one-click email unsubscribe** (RFC 8058 native Gmail/Apple button + per-channel footer links + token-authed public endpoint), **breaking-news pipeline cleanup** (NWS title cleaning, per-user CA-county filter, two-pass cross-source dedupe, live CAL FIRE containment enrichment + live summary override), **fire-perimeter map auto-retires contained fires**, high-risk alerts capped at 20 locations, history page "No fires recorded" empty state, and Gmail-bulletproof email shell.
</details>
