...
export function CensusTractRiskOverlay({ onZoneClick }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      _cachedGeo ? Promise.resolve(_cachedGeo) : apiFetch("/research/boundaries/census-tracts").then((r) => r.json()),
      Object.keys(_cachedRisk).length > 0 ? Promise.resolve({ zones: _cachedRisk }) : apiFetch("/research/risk-by-zone/census-tracts").then((r) => r.json()),
    ])
      .then(([geo, risk]) => {
        if (geo?.features) { _cachedGeo = geo; setGeoData(geo); }
        if (risk?.zones) { _cachedRisk = risk.zones; setRiskData(risk.zones); }
      })
      .catch((e) => console.warn("Census tract overlay load failed:", e));
  }, []);
