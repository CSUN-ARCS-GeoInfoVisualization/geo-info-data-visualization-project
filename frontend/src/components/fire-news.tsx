...
export function FireNews() {
  ...
  const [maxLookbackDays, setMaxLookbackDays] = useState(60);
  const [canExtendLookback, setCanExtendLookback] = useState(true);
  ...
  const loadRecent = useCallback(async () => {
    ...
    try {
      ...
      setLookbackDays(data.lookback_days);
      setMaxLookbackDays(data.max_lookback_days);
      setCanExtendLookback(data.can_extend_lookback);
    } catch {
      setItems([]);
      setHasMoreToLoad(false);
      setCanExtendLookback(false);
    } finally {
  ...
  const loadOlder = useCallback(async () => {
    if (!hasMoreToLoad && !canExtendLookback) return;
    setLoadingOlder(true);
    try {
      const offset = items.length;
      const extendWindow = !hasMoreToLoad && canExtendLookback;
      const requestLookback = extendWindow ? nextNewsLookbackDays(lookbackDays) : lookbackDays;
      const data = await fetchNews("recent", selectedCategory, {
        offset,
        lookback_days: requestLookback,
      });
      setLookbackDays(data.lookback_days);
      setMaxLookbackDays(data.max_lookback_days);
      setCanExtendLookback(data.can_extend_lookback);
  ...
  }, [selectedCategory, hasMoreToLoad, canExtendLookback, items.length, lookbackDays]);
  ...
  const showArticleList =
    !loadingRecent && (items.length > 0 || hasMoreToLoad || canExtendLookback);
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Loads the last 14 days first for speed; use Load more for the next page or to extend
            history in 7-day steps up to {maxLookbackDays} days. Official feeds plus web discovery
            when configured. New URLs are saved for training; duplicates are skipped.
          </p>
        </div>
        <DropdownMenu>
      ...
      {loadingRecent ? (
        ...
        <div className="space-y-6">
          {!loadingRecent && items.length === 0 && !hasMoreToLoad && (
            <Card>
              <CardContent className="pt-6 text-center">
                <Newspaper className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                <h3 className="text-lg font-medium mb-2">
                  {canExtendLookback
                    ? `No fire news in the last ${lookbackDays} days`
                    : `No fire news in the last ${maxLookbackDays} days`}
                </h3>
                {canExtendLookback ? (
                  <p className="text-muted-foreground text-sm">
                    Use Load more to search further back in 7-day steps (up to {maxLookbackDays}{" "}
                    days).
                  </p>
                ) : null}
              </CardContent>
          ...
          {showArticleList && (
            <div className="text-center pt-2">
              <Button
                ...
                disabled={loadingOlder || (!hasMoreToLoad && !canExtendLookback)}
              >
                {loadingOlder ? (
                  <>
              ...
              <p className="text-xs text-muted-foreground mt-2 max-w-md mx-auto">
                {hasMoreToLoad
                  ? "Next page of articles in the current time window (feeds + web discovery)."
                  : canExtendLookback
                    ? `Extend history by 7 days (now showing ${lookbackDays} days, up to ${maxLookbackDays}).`
                    : `Showing up to ${maxLookbackDays} days; no more pages in this category.`}
              </p>
            </div>
          )}
